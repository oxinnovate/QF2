...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        model_args.add('qfmode')
        model_args.add('qffolder')
        model_args.add('sys_qr_len')
        model_args.add('qf_assistant_meta')
        model_args.add('qfbeta')
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if do_sample:
                ...
                # QF-instruct or QF-update mode时需要填充next_tokens而不是预测
                if model_kwargs.get('qfmode') == 'QF-instruct' or model_kwargs.get('qfmode') == 'QF-update'  :
                    position_id=model_inputs['position_ids'][0,-1].item()- model_kwargs['sys_qr_len']+1  
                    token_id=model_kwargs['qf_assistant_meta']['assistant_ids'][position_id]
                    next_tokens = torch.tensor([token_id], device=next_tokens.device)
                    # try:
                    #     next_tokens = torch.tensor([token_id], device=next_tokens.device)
                    # except:
                    #     next_tokens = torch.tensor([151645], device=next_tokens.device)
            ...
            if False:
                ...
                print(f"probs: {probs[0][56994].item():.4f}")
                
                next_tokens = torch.tensor([374], device=next_tokens.device)   #assistant
